import { Card, WingBlank, WhiteSpace, Result, Icon, Modal, Button, Toast, List } from 'antd-mobile';
...
const My = (props: IProps) => {
  ...
  const onClickSetup = (username: string, password: string) => {
    return new Promise<void>((resolve, reject) => {
      ...
      authStore.setUp({ username, password })
        .then(res => {
          console.log(res)
          Toast.hide()
  ...
  const onClickSetupBtn = () => {
    let setupModal = Modal.prompt(
      <div>注册<span style={{ float: 'right' }} onClick={() => setupModal.close()}>🔙</span></div>,
      '请输入账号和密码',
      [
        { text: '取消' },
        { text: '提交注册', onPress: (username, password) => onClickSetup(username, password) }
      ],
      'login-password',
      undefined,
      ['请输入账号', '输入密码'],
  ...
  const onClickLoginBtn = () => {
    let loginModal = Modal.prompt(
      <div>注册<span style={{ float: 'right' }} onClick={() => loginModal.close()}>🔙</span></div>,
      '请输入账号和密码',
      [
        { text: '取消' },
        { text: '登录', onPress: (username, password) => onClickLogin(username, password) }
      ],
      'login-password',
      undefined,
      ['请输入账号', '输入密码'],
        ...
        {authStore.hasLogin ? (
          <Card>
            <Card.Header
              ...
              extra={<div className="position" onClick={onClickLogout}>
                退出 <Icon type="right" />
              </div>}
            />
            <Card.Body>
            ...
            <Card.Footer content={<List.Item onClick={onClickSettings} thumb={<div className="settings"></div>}>设置</List.Item>} />
          </Card>
        ) : (
          <Card className="fake-card">
            ...
            />
            <Card.Body>
              <Button type="ghost" className="my-btn" onClick={onClickSetupBtn}>setup</Button>
              <Button type="primary" className="my-btn" onClick={onClickLoginBtn}>login</Button>
            </Card.Body>
          </Card>
        )}
